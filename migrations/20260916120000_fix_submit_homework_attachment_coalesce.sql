...
begin
  ...
  loop
    if pg_catalog.jsonb_typeof(v_attachment) <> 'object'
       or coalesce(pg_catalog.btrim(v_attachment ->> 'storage_path'), '') = ''
       or coalesce(pg_catalog.btrim(v_attachment ->> 'original_name'), '') = ''
       ...
       or coalesce(pg_catalog.btrim(v_attachment ->> 'mime_type'), '') = ''

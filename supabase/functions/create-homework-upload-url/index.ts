...
type ErrorCode = "invalid_request" | "invalid_platform_auth" | "course_access_denied" |
  ...
  "attachment_type_not_allowed" | "unsupported_file_type" | "file_too_large" |
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const { data: homework, error: homeworkError } = await supabase.from("lesson_homeworks")
      .select("id, lesson_id, is_enabled, allowed_response_types").eq("id", input.homework_id).eq("is_enabled", true).maybeSingle();
    ...
    if (!Array.isArray(homework.allowed_response_types) || !homework.allowed_response_types.includes(input.attachment_type)) {
      throw new RequestError("attachment_type_not_allowed", 400);
    }

    const rules = FILE_RULES[input.attachment_type];
    ...
    if ((input.size_bytes as number) > rules.maxSize) throw new RequestError("file_too_large", 400);
    ...
    const storagePath = `pending/courses/${context.courseId}/students/${context.productUserId}/homeworks/${input.homework_id}/${crypto.randomUUID()}`;
    try {
      const s3 = new S3Client({ endpoint, region, credentials: { accessKeyId, secretAccessKey } });
      const command = new PutObjectCommand({ Bucket: bucket, Key: storagePath, ContentType: mimeType });
      const uploadUrl = await getSignedUrl(s3, command, { expiresIn: EXPIRES_IN });
      return jsonResponse({ ok: true, upload: {
        url: uploadUrl, storage_path: storagePath, method: "PUT", content_type: mimeType,
        expires_in: EXPIRES_IN, max_size_bytes: rules.maxSize,
      } });
    } catch (_error) {
      // SDK errors and signed URLs can contain sensitive request details; log neither.
      console.error("Homework upload URL signing failed");
    ...
    console.error("Create homework upload URL failed", { error_type: error instanceof Error ? error.name : "UnknownError" });
